...
import pandas as pd
from .. import cfg
import cluster_expansion_symmetry as ces
import cluster_expansion_periodic as cep
import bond_counting as bc
from tqdm import tqdm


def build_pd_file(element_set, path="../data"):
    reference_config = cfg.read_config(os.path.join(path, "raw/config0", "start.cfg"))
    ...
    df = pd.DataFrame.from_dict(
        ...
        columns=["index", "migration_atom", "migration_system", "migration_barriers", "energy_difference",
                 ...
                 "cluster_expansion_change_forward", "cluster_expansion_change_backward",
                 "distance_list", "energy_list"])
    df.to_pickle(os.path.join(path, "processed", "all_compiled_data_new.pkl"))
