...
def get_sorted_atom_vectors(config: Config):
    move_distance = -config.atom_list[get_vacancy_index(config)].relative_position
...
def get_average_cluster_parameters_mapping(config: Config) -> typing.List[typing.List[typing.List[int]]]:
    atom_list = get_sorted_atom_vectors(config)
...
if __name__ == "__main__":
    config11 = read_config("../test/test_files/test.cfg")
    cl_mapping = get_average_cluster_parameters_mapping(config11)
    ce = get_one_hot_encoding_list_from_map(config11, get_vacancy_index(config11), {"Al", "Mg", "Zn"}, cl_mapping)
    print(ce)
